...
;
; cat song.inc | grep module_func | cut -b5- | tr -d , | xargs -n1 | sort -u
; 
...
; MODULE_FUNC(kbd)
module_func_kbd:
	fld	dword [pitch+edx] ;*4] ; only place where channel nbr in edx is used
	ret


module_func_modulator:
	fld     dword [pitch+eax*4] ; pull pitch from another channel
	ret
...
;	call	hardrestart
	...
	fmul	dword [half]    ; scv 1.0 pwm sub nse acc
...
;;	jnz	.vco_sine
;	fld	st3
;	fld	dword [threefourths]
;	fcomip	st0, st1
;	fmul	dword [four]	;4*acc
;	jc	.vco_trilastq
;	fld1
;	jmp	.vco_tricalc
;.vco_trilastq:
...
.vco_suboscdone:
	...
	fadd st0, st2
	; sub+osc  pwm  osc  noise  acc

;	; noise
	fild	dword [noise] ; noise  sub+osc  pwm  osc  noise_level  acc
	fmul	st0, st4
	fdiv 	dword [noise_div]
	fadd	st1
...
;	push	eax
	...
	jnz	.delay_bufok
	movzx	eax, byte [delaycount]
	shl	eax, 18 ; = imul eax, eax, DELAYBUFFERSIZE
	lea	edi, [delaybuffer+eax*4]
	inc	byte [delaycount]
	mov	[esi], edi
.delay_bufok:
	mov	ebx, [esi+4]
